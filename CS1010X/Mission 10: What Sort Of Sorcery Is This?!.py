...
def is_empty_tree(tree):
    # Your code here
    if type(tree) == list and len(tree) >= 3:
        return False
    else:
        return True
...
def insert_tree(x, tree):
    ...
    elif x <= entry(tree) and is_empty_tree(left_branch(tree)):
        return build_tree(entry(tree), build_tree(x, make_empty_tree(), make_empty_tree()), right_branch(tree))
    ...
    elif x > entry(tree) and is_empty_tree(right_branch(tree)):
        return build_tree(entry(tree), left_branch(tree), build_tree(x, make_empty_tree(), make_empty_tree()))
...
# Time complexity of insert_tree: O(n^2 log n)
...
def flatten(tree):
    ...
    if not is_empty_tree(left_branch(tree)):
        flattened_list.append(flatten(left_branch(tree)))
    flattened_list.append(entry(tree))
    if not is_empty_tree(right_branch(tree)):
        flattened_list.append(flatten(right_branch(tree)))
